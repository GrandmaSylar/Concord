...
import { ShieldAlert, Terminal, Palette, Image as ImageIcon, Check, Loader2, LogOut, Smartphone, Send, CheckCircle2, AlertTriangle, HelpCircle, RefreshCw, Zap, Play, Trash2, BookOpen, Layers, Cpu, ChevronDown, ChevronUp, StopCircle, Timer, Gauge, Activity, Users, UserPlus, Edit, Trash, Plus, Key, Landmark } from 'lucide-react'
import Link from 'next/link'
import { getSystemSettings, updateSystemSettings, sendTestSMS, getRecentSMSLogs, SystemSettings, prepareSimulationQueue, getSimulationStats, triggerSimulationRun, clearSimulationQueue, haltSimulation, listSystemUsers, createSystemUser, editSystemUser, deleteSystemUser, type SystemUser } from '@/app/actions/settings'
...
export default function DevSettingsPage() {
 ...
 const [userForm, setUserForm] = useState<{ email: string; password?: string; full_name: string; role: 'admin' | 'user' }>({
 email: '',
 password: '',
 full_name: '',
 role: 'user',
 ...
 try {
 const res = await createSystemUser(userForm)
 if (res.success) {
 ...
 setUserSubmitting(true)
 try {
 const res = await editSystemUser(selectedUser.id, userForm)
 if (res.success) {
 ...
 if (!confirm('Are you absolutely sure you want to delete this user? This will delete all of their data, including contacts they own.')) return
 ...
 className="flex items-center gap-2 px-4 py-2 bg-blue-50 text-blue-700 font-semibold rounded-lg hover:bg-blue-100 transition-colors text-sm border border-blue-200 cursor-pointer"
 >
 <Landmark className="w-4 h-4 text-blue-600" /> Apply NPP Preset
 </button>
 </div>
 </div>
 ...
 <button 
 onClick={applyNPPPreset}
 className="flex items-center justify-center gap-2 px-4 py-2.5 bg-blue-50 text-blue-700 font-semibold rounded-lg hover:bg-blue-100 transition-colors text-sm border border-blue-200 cursor-pointer"
 >
 <Landmark className="w-4 h-4 text-blue-600" /> Apply NPP Preset
 </button>
 <button 
 ...
 <div>
 <h2 className="text-xl font-bold text-slate-900">System User Directory</h2>
 <p className="text-sm text-slate-500">Manage user accounts, roles, and administrative permissions.</p>
 </div>
 </div>
 <button
 onClick={handleOpenCreateModal}
 className="flex items-center gap-2 px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-semibold rounded-lg shadow-sm transition-colors cursor-pointer"
 >
 ...
 <Users className="w-8 h-8 text-slate-400 mx-auto mb-2.5 opacity-50" />
 <p className="text-sm font-semibold text-slate-700">No users found</p>
 <p className="text-xs text-slate-500 mt-1">Add users to allow staff members to manage campaigns.</p>
 </div>
 ) : (
 ...
 <tr>
 <th scope="col" className="py-3 px-4 text-xs font-semibold text-slate-500 uppercase tracking-wider">User Details</th>
 <th scope="col" className="py-3 px-4 text-xs font-semibold text-slate-500 uppercase tracking-wider">Role</th>
 <th scope="col" className="py-3 px-4 text-xs font-semibold text-slate-500 uppercase tracking-wider">Joined Date</th>
 <th scope="col" className="py-3 px-4 text-xs font-semibold text-slate-500 uppercase tracking-wider text-right">Actions</th>
 </tr>
 </thead>
 ...
 <div className="flex items-center gap-3">
 <div className="w-9 h-9 rounded-full bg-indigo-50 border border-indigo-100 text-indigo-700 font-extrabold flex items-center justify-center text-sm">
 ...
 <div className="bg-white rounded-xl shadow-xl border border-slate-200 max-w-md w-full p-6 animate-in zoom-in-95 duration-200 relative">
 ...
 type="email"
 required
 placeholder="e.g. user@concord.org"
 ...
 <input
 type="password"
 placeholder="Defaults to Concord123!"
 value={userForm.password}
 onChange={(e) => setUserForm({ ...userForm, password: e.target.value })}
 className="w-full bg-slate-50 border border-slate-200 rounded-lg py-2 pl-9 pr-3 text-sm text-slate-900 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all font-mono"
 />
 </div>
 </div>
 ...
 className="w-full bg-slate-50 border border-slate-200 rounded-lg px-3 py-2 text-sm text-slate-900 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all"
 >
 <option value="user">Staff / Campaign User</option>
 <option value="admin">Administrator (Bypasses RLS)</option>
 </select>
 </div>
 ...
 className="flex items-center gap-1.5 px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-semibold rounded-lg shadow-sm transition-colors disabled:opacity-50 cursor-pointer"
 >
 {userSubmitting && <Loader2 className="w-3.5 h-3.5 animate-spin" />}
 Create User
 </button>
 </div>
 </form>
 ...
 <div>
 <label className="block text-sm font-semibold text-slate-700 mb-1">Override Password</label>
 <div className="relative">
 <Key className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400" />
 <input
 type="password"
