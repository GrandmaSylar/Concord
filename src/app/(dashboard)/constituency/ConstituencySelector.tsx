...
import { Search, X, Users, Phone, MapPin, ChevronLeft, ChevronRight, Send, Filter, AlertTriangle, Landmark, CheckCircle2, Loader2 } from 'lucide-react'
...
export default function ConstituencySelector({ initialContacts, initialTotal, groups, initialStats }: Props) {
 ...
 <p className="text-lg font-bold text-gray-900">{s.value}</p>
  ...
  <div className="flex border-b border-gray-200 overflow-x-auto relative scrollbar-none bg-slate-50/50 p-1 gap-1">
  {TABS.map(tab => (
  <button
  key={tab}
  onClick={() => { setActiveTab(tab); if (tab === 'All Members') clearAllFilters() }}
  className={`relative px-4 py-2 text-xs sm:text-sm font-semibold rounded-md whitespace-nowrap transition-colors duration-200 cursor-pointer ${
  activeTab === tab
  ...
  {activeTab === tab && (
    <motion.div
      ...
      className="absolute inset-0 bg-white shadow-xs border border-slate-200/50 rounded-md z-0"
 ...
 Exclude contacts without phone
  ...
  <button 
    ...
    className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-semibold bg-red-50 text-red-600 border border-red-150 hover:bg-red-100 hover:border-red-200 transition-all active:scale-95 cursor-pointer"
  >
    <X className="w-3 h-3" />
    Clear all
  </button>
  ...
  <div className="px-4 pb-2 flex flex-wrap items-center gap-2">
    ...
    {selected.size > 0 && (
      <button 
        ...
        className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold bg-red-50 text-red-700 border border-red-200 hover:bg-red-100 hover:border-red-300 transition-all hover:scale-[1.02] active:scale-95 shadow-xs cursor-pointer"
      >
