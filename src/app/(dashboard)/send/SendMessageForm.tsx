...
import { Send, Search, ChevronLeft, ChevronRight, Filter, UserPlus, X, Sparkles, AlertCircle, CheckCircle2, Smartphone, Loader2 } from 'lucide-react'
...
export default function SendMessageForm({ 
 ...
 const allSelected = contacts.every(c => selectedContacts.has(c.phone))
 const next = new Map(selectedContacts)
 if (allSelected) {
 ...
 disabled={totalRecipientsCount === 0}
 className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-semibold bg-gradient-to-r from-violet-500 to-fuchsia-500 text-white shadow-sm hover:shadow-md hover:scale-[1.03] active:scale-95 transition-all cursor-pointer disabled:opacity-40"
 >
 ...
 <AIAssistantWidget
 onInsertDraft={(draft) => setMessage(draft)}
 context={{
 position: filterPosition || undefined,
 subArea: filterSubArea || undefined,
 }}
 isOpen={showAI}
 setIsOpen={setShowAI}
 />
