...
export async function listSystemUsers(): Promise<{ success?: boolean; error?: string; users?: SystemUser[] }> {
 try {
 ...
 const combined: SystemUser[] = (authData?.users || []).map(user => {
 ...
 return { success: true, users: combined }
 } catch (err: any) {
 console.error('Unexpected error in listSystemUsers:', err)
 return { error: err.message || 'Unexpected error' }
 }
}

export async function createSystemUser(data: { email: string; password?: string; full_name: string; role: 'admin' | 'user' }) {
 try {
 const { data: userDetails, error: authError } = await adminSupabase.auth.admin.createUser({
 email: data.email,
 password: data.password || 'Concord123!',
 email_confirm: true,
 ...
 // Upsert the profile record to prevent timing conditions with the on-create trigger
 ...
 console.error('Unexpected error in createSystemUser:', err)
 return { error: err.message || 'Unexpected error' }
 }
}

export async function editSystemUser(userId: string, data: { email: string; password?: string; full_name: string; role: 'admin' | 'user' }) {
 try {
 ...
 if (authError) {
 console.error('Error editing auth user:', authError)
 return { error: authError.message }
 }
 ...
 console.error('Unexpected error in editSystemUser:', err)
 return { error: err.message || 'Unexpected error' }
 }
}
